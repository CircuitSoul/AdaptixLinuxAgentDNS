...
func init() {
	f, err := os.OpenFile("/tmp/dnslistener.log", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err == nil {
...
func (t *TransportDNS) handleHB(req *dnsRequest) (needsReset bool, hasPendingTasks bool) {
	...
	// Backward compatibility with older agent builds that accidentally appended
	// the ack offset after the 12-byte HB buffer (bytes 12..15). In that case
	// bytes 0..3 are zero; recover the real offset so delivery can be acked.
	if len(decrypted) >= 16 && ackOffset == 0 {
		ackOffset = binary.BigEndian.Uint32(decrypted[12:16])
	}
...
func (t *TransportDNS) handlePUT(req *dnsRequest) putAckInfo {
	...
	decrypted := rc4Crypt(req.data, t.Config.EncryptKey)
	dlog.Printf("DNS PUT sid=%s raw=%d dec=%d qtype=%d", req.sid, len(req.data), len(decrypted), req.qtype)
	ack = t.handlePutFragment(req.sid, req.seq, decrypted, ack)
	dlog.Printf("DNS PUT ack sid=%s total=%d filled=%d next=%d complete=%v reset=%v", req.sid, ack.total, ack.filled, ack.nextExpectedOff, ack.complete, ack.needsReset)
...
func (t *TransportDNS) handlePutFragment(sid string, seq int, data []byte, ack putAckInfo) putAckInfo {
	...
	if offset == 0 && total <= uint32(len(chunk)) {
		dec := decompressUpstream(chunk)
		dlog.Printf("DNS PUT single sid=%s total=%d chunk=%d dec=%d meta=%v", sid, total, len(chunk), len(dec), hasMeta)
		if err := Ts.TsAgentProcessData(sid, dec); err != nil {
			dlog.Printf("DNS ProcessData ERROR sid=%s err=%v", sid, err)
		}
		ack.complete = true
		ack.filled = total
	...
	if completeBuf != nil {
		dlog.Printf("DNS PUT multi-complete sid=%s total=%d", sid, total)
		if err := Ts.TsAgentProcessData(sid, decompressUpstream(completeBuf)); err != nil {
			dlog.Printf("DNS ProcessData MULTI ERROR sid=%s err=%v", sid, err)
		}
	}
